"""
Errors Module for Claude Pet Companion
...
# ============================================================================
# Exception Classes (redefined here to avoid circular import)
# ============================================================================
...
class PetFileNotFoundError(PetError):
...
# Export old errors module functions for backward compatibility
# Import the standalone errors.py for any additional functions
try:
    from .. import errors as standalone_errors
    # Share the same logger instances
    _default_logger = standalone_errors._default_logger if hasattr(standalone_errors, '_default_logger') else None
...
def setup_error_handling(crash_dir: Path, save_dir: Path,
                        auto_restart: bool = False) -> tuple:
    """
    ...
    Args:
        ...
        auto_restart: Whether to enable auto-restart on crash

    Returns:
        Tuple of (crash_handler, auto_save_manager)
    """
    # Setup logging
    init_logging(save_dir)

    # Setup crash handler
    ch = setup_crash_handler(crash_dir, auto_restart=auto_restart)

    # Setup auto-save
    asm = setup_auto_save(save_dir)

    return ch, asm


def handle_crash(exc: Exception, context: dict = None) -> CrashReport:
    """
    Handle an exception with full crash reporting.
    ...
    # Log error
    log_error(exc, get_default_logger(), context)

    # Save crash report
    if crash_handler.crash_dir:
        filename = f"crash_{int(report.timestamp)}.json"
        filepath = crash_handler.crash_dir / filename
